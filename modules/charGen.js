...
function CharacterGenerator() {
  ...
  function rollCharacter() {
    ...
    } else if (["Shortbow", "Longbow", "Warbow"].includes(weapon.name)) {
      ...
      <div className="w-full max-w-3xl">
        <Card>
          <CardHeader>
            ...
            <Button onClick={rollCharacter}>New Character</Button>
          </CardHeader>
          <CardContent>
            {pc ? (
              <CharacterSheet
              ...
              <p className="text-center italic text-gray-600">Click “New Character” to begin.</p>
            )}
          </CardContent>
        </Card>
      </div>
...
function CharacterSheet({
  ...
  function getAmmoForWeapon(weaponName) {
    ...
    if (["Shortbow", "Longbow", "Warbow"].includes(weaponName)) return "Quiver of Arrows x20";
    return null;
  }
  ...
  useEffect(() => { setNameInputValue(pc.name); }, [pc.name]);
