...
const Card = ({ children }) => (
  <div className="bg-white rounded-xl shadow-md p-4 w-full">{children}</div>
);
...
function CharacterGenerator(){
  ...
  function rollCharacter(){
    ...
    const attrs=attributeOrder.map(a=>{const s=scores[a];const m=mod(s);return{attr:a,score:s,mod:m,primary:primaries.has(a),check:m+(primaries.has(a)?1:0)};});
    ...
    const totalSlots=inventory.reduce((s,i)=>s+i.slots,0);
...
const Field  =({label,value})=>(<div><span className="block text-xs uppercase tracking-wide text-gray-500">{label}</span><span className="block font-medium break-words">{value}</span></div>);
    ...
    <div className="space-y-6">
      ...
      <section>
        ...
        <ul className="list-disc list-inside">
          {pc.inventory.map((it,i)=>(<li key={i}>{it.name} — <span className="italic text-gray-400">{it.slots} slot{it.slots!==1?"s":""}</span></li>))}
        </ul>
      </section>
